...
-- Helper: total cash purchases for a business date (restaurant-wide)
CREATE OR REPLACE FUNCTION public.cash_expense_total(_business_date date)
RETURNS numeric
LANGUAGE sql
...
-- Atomic upsert for daily cash reconciliation
...
BEGIN
  ...
  IF _finalise THEN
    ...
    INSERT INTO public.audit_log(restaurant_id, actor_id, action, entity_type, entity_id, details)
...
GRANT EXECUTE ON FUNCTION public.cash_expense_total(date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.save_cash_reconciliation(date, text, jsonb, jsonb, boolean) TO authenticated;
